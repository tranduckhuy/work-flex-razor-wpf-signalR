...
namespace WorkFlex.Infrastructure.Repositories
{
    public class JobRepository : IJobRepository
    {
        ...
        public async Task<(IEnumerable<JobPost> JobPosts, Pageable<SearchCriteria> pageable)> GetJobsByUserIdAsync(int page,
            Guid userId, SearchCriteria searchCriteria)
        {
            ...
            if (searchCriteria != null && !string.IsNullOrEmpty(searchCriteria.SearchValue))
            {
                ...
                {
                    case nameof(AppConstants.JobPostSearchOption.Title):
                        query = query.Where(jp =>
                            EF.Functions.Like(
                                EF.Functions.Collate((jp.Title), "Latin1_General_CI_AI"),
                                $"%{searchValue}%"
                            )
                        );
                        break;

                    case nameof(AppConstants.JobPostSearchOption.Location):
                        query = query.Where(jp =>
                            EF.Functions.Like(
                                EF.Functions.Collate((jp.JobLocation), "Latin1_General_CI_AI"),
                                $"%{searchValue}%"
                            )
                        );
                        break;
